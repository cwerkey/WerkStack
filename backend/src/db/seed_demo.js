...
// Demo seed script — creates a site with 2 zones, 2 racks, and 50 devices.
// Requires the base seed.js to have run first (org + user must exist).
//
// Usage (local):  DATABASE_URL=... node backend/src/db/seed_demo.js
// Usage (Docker): see README instructions below
...
async function seedDemo() {
  ...
  await migrate(db);
  console.log('[seed-demo] migrations applied');

  const client = await db.connect();
  try {
    // Require base org to exist
    const orgResult = await client.query(`SELECT id FROM organizations LIMIT 1`);
    if (orgResult.rows.length === 0) {
      console.error('[seed-demo] no organization found — run seed.js first');
      process.exit(1);
    }
    const orgId = orgResult.rows[0].id;

    // Skip if demo site already exists
    const existing = await client.query(
      `SELECT id FROM sites WHERE org_id = $1 AND name = 'Demo Lab'`,
      [orgId]
    );
    if (existing.rows.length > 0) {
      console.log('[seed-demo] demo data already exists — skipping');
      console.log('           Delete the "Demo Lab" site to re-run.');
      await db.end();
      return;
    }
    ...
    const siteResult = await client.query(
      `INSERT INTO sites (org_id, name, location, color, description)
       VALUES ($1, 'Demo Lab', 'Server Room A', '#4a90d9', '50-device demo environment with 2 racks')
       RETURNING id`,
      [orgId]
    );
    ...
    console.log('');
    console.log('  Site:    Demo Lab  (Server Room A)');
    console.log('  Zone 1:  Primary Compute   → Rack A — 28 devices');
    console.log('  Zone 2:  Storage & Svc     → Rack B — 22 devices');
    console.log('  Total:   50 devices');
    console.log('');
    console.log('  Statuses seeded:');
    console.log('    up          — most devices');
    console.log('    degraded    — web-03, fw-02');
    console.log('    down        — plex-01');
    console.log('    maintenance — test-03');
  } catch (err) {
